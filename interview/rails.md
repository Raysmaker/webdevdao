...
1. Как сгенерировать модель, конторллер, представление (вьюху)
    <details>
      <summary>Ответ</summary>
      * `rails generate model YourName` — Модель. Имя модели обязательно в единственном числе, Car - правильно, Cars - не правильно!
      
      * `rails generate controller YourName` — Контроллер.
      * `rails generate view YourName` — Вид.
    </details>
